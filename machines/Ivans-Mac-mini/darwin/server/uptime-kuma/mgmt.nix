...

{
  ...
  local.services.uptime-kuma-mgmt = {
    enable = true;
    # Connect to local instance using machineBindAddress
    baseUrl = "http://${config.flags.machineBindAddress}:3001";
    ...
    monitors = [
      ...
      {
        name = "prowlarr";
        url = "http://${config.flags.machineBindAddress}:9696";
        description = "Prowlarr indexer manager";
      }
      {
        name = "radarr";
        url = "http://${config.flags.machineBindAddress}:7878";
        description = "Radarr movie manager";
      }
      {
        name = "sonarr";
        url = "http://${config.flags.machineBindAddress}:8989";
        description = "Sonarr TV manager";
      }
      {
        name = "transmission";
        url = "http://${config.flags.machineBindAddress}:9091";
        expectedStatus = 401;
        description = "Transmission torrent client (RPC auth required)";
      }
      {
        name = "jellyfin";
        url = "http://${config.flags.machineBindAddress}:8096";
        description = "Jellyfin media server";
      }
      {
        name = "stash";
        url = "http://${config.flags.machineBindAddress}:9999";
        description = "Stash media organizer";
      }
      {
        name = "media";
        url = "http://${config.flags.machineBindAddress}:9998";
        description = "Stash media organizer (general)";
      }
      {
        name = "audiobookshelf";
        url = "http://${config.flags.machineBindAddress}:8000";
      ...
      {
        name = "syncthing";
        url = "http://${config.flags.machineBindAddress}:8384";
        description = "Syncthing file sync";
      }
      {
        name = "beszel";
        url = "http://${config.flags.machineBindAddress}:8091";
        description = "Beszel monitoring hub";
      }
      {
        name = "miniserve";
        url = "http://${config.flags.machineBindAddress}:8080";
        expectedStatus = 401;
        description = "Miniserve file server (auth required)";
      }
      {
        name = "bin";
        url = "http://${config.flags.machineBindAddress}:8820";
        description = "Pastebin service";
      }
      {
        name = "podservice";
        url = "http://${config.flags.machineBindAddress}:8083";
        description = "YouTube to Podcast service";
      }
      {
        name = "textcast";
        url = "http://${config.flags.machineBindAddress}:8084";
        description = "Article to audiobook service";
      }
      {
        name = "youtube";
        url = "http://${config.flags.machineBindAddress}:8085";
        description = "YouTube video downloader";
      }
      {
        name = "matrix";
        url = "http://${config.flags.machineBindAddress}:8009";
        interval = 30;
        description = "Matrix Synapse server (critical)";
      }
      {
        name = "uptime-kuma";
        url = "http://${config.flags.machineBindAddress}:3001";
        description = "Uptime Kuma monitoring";
      }
      {
        name = "doh";
        url = "http://${config.flags.machineBindAddress}:8053/dns-query?dns=AAABAAABAAAAAAAAA3d3dwdleGFtcGxlA2NvbQAAAQAB";
        expectedStatus = 200;
        description = "DNS over HTTPS service";
      }
      {
        name = "ollama";
        url = "http://${config.flags.machineBindAddress}:11434";
        description = "Ollama LLM API";
      }
      {
        name = "openwebui";
        url = "http://${config.flags.machineBindAddress}:8090";
      ...
      {
        name = "grafana";
        url = "http://${config.flags.machineBindAddress}:3000";
        description = "Grafana dashboard";
      }
      {
        name = "loki";
        url = "http://${config.flags.machineBindAddress}:3100/ready";
      ...
      {
        name = "promtail";
        url = "http://${config.flags.machineBindAddress}:9080/ready";
      ...
      # DNS Infrastructure
      {
        name = "dnsmasq";
        type = "dns";
        url = "example.com@${config.flags.machineBindAddress}";
        interval = 60;
      ...
      }
      {
        name = "stubby";
        type = "tcp";
        url = "${config.flags.machineBindAddress}:5453";
        interval = 60;
      ...
      # Reverse Proxy Infrastructure
      {
        name = "caddy-http";
        type = "tcp";
        url = "${config.flags.machineBindAddress}:80";
        interval = 60;
      ...
      }
      {
        name = "caddy-https";
        type = "tcp";
        url = "${config.flags.machineBindAddress}:443";
        interval = 60;
      ...
      # System Services
      {
        name = "ssh";
        type = "tcp";
        url = "${config.flags.machineBindAddress}:22";
        interval = 60;
      ...
      }
      {
        name = "smb";
        type = "tcp";
        url = "${config.flags.machineBindAddress}:445";
        interval = 60;
