{ config, pkgs, ... }:
{
  services.postgresql = {
    ...
    dataDir = "${config.flags.externalStoragePath}/.postgresql/${config.services.postgresql.package.psqlSchema}";
    enableTCPIP = false;
    port = 5433;
  ...
  launchd.user.agents.postgresql = {
    ...
    script = pkgs.lib.mkBefore ''
      ...
      echo "Waiting for ${config.flags.externalStoragePath} to be available..."
      /bin/wait4path "${config.flags.externalStoragePath}"
      echo "${config.flags.externalStoragePath} is now available!"
    '';
  };
}
