...
let
  ...
  wrapPythonScript = scriptName: config:
    let binaryName = pkgs.lib.removeSuffix ".py" scriptName;
    in pkgs.writeShellScriptBin binaryName ''
      exec ${pkgs.python3.withPackages config.deps}/bin/python ${scriptPath}/${scriptName} "$@"
    '';

  # Create packages for scripts with dependencies
  depsPackages = builtins.attrValues (builtins.mapAttrs (name: config:
