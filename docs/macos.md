...
Query the TCC database to see which applications and binaries have Full Disk
Access:
...
Note: System binaries in `/bin`, `/usr/bin` with the `restricted` flag
(SIP-protected) cannot be granted Full Disk Access. Use `ls -lO /path/to/binary`
to check for the `restricted` flag.
