...
After updating `.sops.yaml` (adding/removing keys) or when sops-nix
auto-discovers a new SSH host key during rebuild, re-encrypt the secrets file:
...
On macOS, `sops-install-secrets` requires **Full Disk Access** permission to
read the SSH host key at `/etc/ssh/ssh_host_ed25519_key`.
