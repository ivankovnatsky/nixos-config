...
in
{
    ...
    local.services.mautrix-discord = {
      ...
      environmentFile = lib.mkOption {
        ...
        description = ''
          ...
          out of Nix store to the `local.services.mautrix-discord.dataDir`.
          Can be used for storing the secrets without making them available in the Nix store.
          For example, you can set `local.services.mautrix-discord.settings.appservice.as_token = "$MAUTRIX_DISCORD_APPSERVICE_AS_TOKEN"`
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Disable meta.doc to avoid NixOS manual build failures with local module
    # meta = {
