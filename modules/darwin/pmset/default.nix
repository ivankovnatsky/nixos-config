    ...
    schedules = mkOption {
      ...
      type = types.attrsOf (types.submodule {
        options = {
          ...
          action = mkOption {
            type = types.enum [ "sleep" "shutdown" "restart" "wakeorpoweron" ];
    ...
    system.activationScripts.postActivation.text = mkAfter (
      if cfg.enable then
        let
          ...
          generateAllSchedules = schedules: 
            let
              ...
              scheduleParams = concatStringsSep " " (
                mapAttrsToList formatSchedule schedules
              );
            in
            ...
            '';
        in generateAllSchedules enabledSchedules
      else
        ''
