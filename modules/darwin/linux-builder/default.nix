# Forked from nix-darwin's modules/nix/linux-builder.nix
# Ref: https://github.com/nix-darwin/nix-darwin/issues/1505
#
# Changes from upstream:
# - Options under `local.linux-builder` (was `nix.linux-builder`)
# - Removed `nix.enable` assertion (Determinate Nix compatibility)
# - Removed mkRemovedOptionModule import
...
let
  cfg = config.local.linux-builder;
in

{
  options.local.linux-builder = {
  ...
  config = mkMerge [
    ...
    (mkIf cfg.enable {
      ...
      system.activationScripts.preActivation.text = ''
        ...
        # Generate SSH keys for linux-builder if they don't exist
        # (upstream relies on create-builder calling sudo for this,
        # which fails in a launchd daemon context)
        if [ ! -e /etc/nix/builder_ed25519 ]; then
          mkdir -p /etc/nix
          ssh-keygen -q -f /etc/nix/builder_ed25519 -t ed25519 -N "" -C 'builder@localhost'
          chown root:nixbld /etc/nix/builder_ed25519
          chmod 600 /etc/nix/builder_ed25519
          chmod 644 /etc/nix/builder_ed25519.pub
        fi

        # Also copy keys to working directory so create-builder
        # sees them and skips the sudo call
        mkdir -p ${cfg.workingDirectory}/keys
        cp -f /etc/nix/builder_ed25519 ${cfg.workingDirectory}/keys/
        cp -f /etc/nix/builder_ed25519.pub ${cfg.workingDirectory}/keys/
      '';
