...
let
  ...
  configJson = pkgs.writeText "logscanner-config.json" (
    builtins.toJSON {
      inherit (cfg) logPaths patterns darwinPredicate;
...
in
{
  options.local.services.logscanner = {
    ...
    darwinPredicate = mkOption {
      type = types.str;
      default = "messageType == error";
      description = "macOS unified log predicate for log show";
    };
  ...
  config = mkIf cfg.enable {
    local.launchd.services.logscanner = {
      ...
      command =
        let
          scanScript = pkgs.writeShellScript "logscanner-run" ''
            set -e
