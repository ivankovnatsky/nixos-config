...
def main():
    ...
    parser.add_argument('input_file', nargs='?', help='File containing CSP data or Terraform diff')
    parser.add_argument('--terraform-diff', '-t', action='store_true', 
                       help='Parse input as Terraform diff output')
    parser.add_argument('--old-file', help='Path to file containing old CSP (alternative to single file)')
    parser.add_argument('--new-file', help='Path to file containing new CSP (alternative to single file)')
        ...
        if args.old_file and args.new_file:
            ...
            if not old_csp or not new_csp:
                ...
                if '\n---\n' in content:
                    ...
                    print("Error: Could not parse Terraform diff format", file=sys.stderr)
                    print("Expected format: ~ content_security_policy = \"old\" -> \"new\"", file=sys.stderr)
                    sys.exit(1)
                else:
                    print("Error: Could not parse file. Expected Terraform diff or separator '---' or '==OLD==' and '==NEW==' markers", file=sys.stderr)
                    print("Tip: Try adding --terraform-diff flag if this is Terraform output", file=sys.stderr)
                    sys.exit(1)
        else:
