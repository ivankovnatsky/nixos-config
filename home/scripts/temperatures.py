...
try:
    subprocess.run(['sensors', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def get_temperatures():
    result = subprocess.run(['sensors', '-j'], stdout=subprocess.PIPE, text=True, check=True)
            ...
            for key, value in sensor_data.items():
                if 'temp' in key and 'input' in key and isinstance(value, (int, float)):
                    ...
                    if sensor_id not in max_temps:
                        ...
                        max_temps[sensor_id]['timestamp'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
def display_temperatures(current_data):
    ...
    sorted_temps = sorted(max_temps.items(), key=lambda x: x[1]['max_temp'], reverse=True)
